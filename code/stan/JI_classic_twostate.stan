...
generated quantities {
  ...
  vector[Tm1] phi_out; // Overall survival probability for all frogs per period
...

  {
    ...
    for (t in 1 : Tm1) {
      N[t] = sum(al[:, t]);
      B[t] = sum(d[:, t]);
      phi_out[t] = mean(inv_logit(X_surv[:, :] * beta_phi + eps_phi[t])); // Compute mean survival across all frogs
    }
