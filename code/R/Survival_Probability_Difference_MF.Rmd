...
mod <- readRDS("/Users/johnimperato/Desktop/Mossy-Pond_companion/sex_survival_covariate_model.rds")
...
# Define a helper function to compute summary statistics
...
time_summaries_list <- lapply(1:T, function(t) {
...
time_summaries_df <- bind_rows(time_summaries_list, .id = "time_index") %>%
  mutate(time_period = paste0("time_", time_index)) %>%
...
# Plot the difference with credible intervals
ggplot(time_summaries_df, aes(x = time_period, y = mean)) +
  geom_point(color = "blue") +
  geom_errorbar(aes(ymin = lower95, ymax = upper95), width = 0.2, color = "blue") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(
    title = "Difference in Survival Probabilities (Male - Female) Over Time",
    x = "Time Period",
...
###############################################################################
# 5. Plot the Differences with Correct Time Order
###############################################################################

# Ensure 'time_period' is an ordered factor (if not already done)
time_summaries_df <- time_summaries_df %>%
  mutate(time_period = factor(time_period, 
                              levels = paste0("time_", 1:T),  # Define the correct chronological order
                              ordered = TRUE))

# Verify the ordering
print(levels(time_summaries_df$time_period))

# Create the plot
difference_plot <- ggplot(time_summaries_df, aes(x = time_period, y = mean)) +
  ...
  scale_x_discrete(limits = paste0("time_", 1:T))  # Ensure correct order on x-axis

# Display the plot
print(difference_plot)

# Save the plot to a file
ggsave("difference_in_survival_probabilities.png", plot = difference_plot, width = 10, height = 6, dpi = 300)
